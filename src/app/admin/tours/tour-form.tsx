...
export function TourForm({ mode, tourId, initial }: Props) {
  ...
  // 地図ピッカーを開いている meeting_point の index（null なら閉じている）
  ...
  // lat/lng を1セットで更新
  const updateMeetingPointCoords = (
    index: number,
  ...
  // 現在開いているピッカーの初期値
  const pickerInitial =
    pickerIndex != null ? form.meeting_points[pickerIndex] : undefined;
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <section className="space-y-5">
          <SectionTitle
            ...
            title="集合場所"
            subtitle="先頭がメイン集合場所として扱われます。各地点に地図で位置を設定すると、ツアー詳細ページに地図が表示されます。"
          />
              ...
              >
                ...
                <Field label="場所名">
                  <input
                    ...
                    value={mp.name}
                    onChange={(e) =>
                      updateMeetingPoint(i, "name", e.target.value)
                    }
                    className={inputCls}
                ...
                <Field label="時刻">
                  <input
                    ...
                    value={mp.time}
                    onChange={(e) =>
                      updateMeetingPoint(i, "time", e.target.value)
                    }
                    className={inputCls}
                    placeholder="例：07:30"
                  />
                ...
                <Field label="補足" hint="任意">
                  <input
                    ...
                    value={mp.note ?? ""}
                    onChange={(e) =>
                      updateMeetingPoint(i, "note", e.target.value)
                    }
                    className={inputCls}
                ...
                <div className="space-y-1.5">
                  ...
                  {hasCoords ? (
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="font-mono text-[12px] text-ink-900 bg-paper-100 border border-line px-2 py-1">
                        {mp.lat!.toFixed(4)}, {mp.lng!.toFixed(4)}
                      </span>
                      <button
                        type="button"
                        onClick={() => setPickerIndex(i)}
                        className="text-[11px] tracking-[0.15em] uppercase text-coral-700 border border-line bg-paper-100 hover:bg-paper-200 px-3 py-1.5 transition"
                      >
                        変更
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          updateMeetingPointCoords(i, undefined, undefined)
                        }
                        className="text-[11px] tracking-[0.15em] uppercase text-ink-500 hover:text-ink-900 transition"
                      >
                        クリア
                      </button>
                    </div>
      ...
      <LocationPicker
        ...
        initialLat={pickerInitial?.lat ?? null}
        initialLng={pickerInitial?.lng ?? null}
        initialName={pickerInitial?.name ?? ""}
        onClose={() => setPickerIndex(null)}
        onSelect={(lat, lng) => {
          if (pickerIndex != null) {
            updateMeetingPointCoords(pickerIndex, lat, lng);
          }
        }}
      />
    </>
